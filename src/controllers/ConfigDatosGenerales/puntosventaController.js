...
export const beginUpdate = async (req, res) => {
  ...
  try {
    ifNoCode(Codigo);
    const result = await beginUpdateQuery(
      req.db,
      user,
      Codigo,
      "pre_puntosventa"
...
export const endUpdate = async (req, res) => {
  ...
  try {
    ifNoCode(Codigo);
    const result = await endUpdateQuery(
      req.db,
      user,
      Codigo,
      "pre_puntosventa"
...
export const deletePuntoDeVenta = async (req, res) => {
  ...
  try {
    const result = await deleteQuery(
      req.db,
      "DELETE FROM pre_puntosventa WHERE Codigo = ?",
      [Codigo],
      "Punto de venta"
    );
    return res.send(result);
...
export const createPuntoDeVenta = async (req, res) => {
  ...
  try {
    const result = await insertQuery(
      req.db,
      "INSERT INTO pre_puntosventa (Nombre, UsuarioAltaRegistro) VALUES (?, ?)",
      [Nombre, user],
      "Punto de venta"
    );
    return res.send(result);
...
export const updatePuntoDeVenta = async (req, res) => {
  ...
  try {
    const result = await updateQuery(
      req.db,
      "UPDATE pre_puntosventa SET Nombre = ? WHERE Codigo = ?",
      [Nombre, Codigo],
      "Punto de venta"
    );
    return res.send(result);
