...
export const endUpdateQueryID = async (db, usuario, codigo, table) => {
    if(!codigo){
        throw {status: false, message: `ID required`}
       }
    try {
        const actualUsuario = await db.query(`SELECT inUpdate FROM ${table} WHERE ID = ?`, 
        {
            replacements: [codigo],
            type: QueryTypes.SELECT
        })
        if(actualUsuario[0].inUpdate === usuario){
            await db.query(`UPDATE ${table} SET inUpdate = NULL WHERE ID = ?`, {
                replacements: [codigo],
                type: QueryTypes.UPDATE
            })
            return {status: true}
